...
function seedData(db: Database.Database): void {
  ...
  for (let i = 0; i < usernames.length; i++) {
    ...
    insertUser.run(
      ...
      `${username.charAt(0).toUpperCase() + username.slice(1)} ${['Smith', 'Johnson', 'Williams', 'Brown', 'Jones'][i % 5]}`,
  ...
  for (let i = 0; i < productNames.length; i++) {
    ...
    insertProduct.run(
      name,
      `High-quality ${name.toLowerCase()} for all your needs`,
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  stats.forEach(({ table, count }) => {
    console.log(`${table.padEnd(15)} : ${(count as any).count} rows`);
  });
